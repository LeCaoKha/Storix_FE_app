...
export const useWarehouses = () => {
  ...
  const assignedWarehouseIds =
    user?.warehouseAssignments?.map((assignment) => assignment.warehouseId) ||
    [];
    ...
    queryFn: async () => {
      ...
      if (
        ...
        (!resolvedUser.warehouseAssignments ||
          resolvedUser.warehouseAssignments.length === 0)
      ) {
        console.log(
          "[useWarehouses] refreshing profile because persisted session has no warehouse info",
          {
        ...
        try {
          ...
          resolvedUser = mergeUserProfileIntoUser(resolvedUser, {
            ...(freshUserSummary.status === "fulfilled"
              ? freshUserSummary.value
              : {}),
          ...
          console.log("[useWarehouses] refreshed profile", {
            ...
            assignedWarehouseIds:
              resolvedUser.warehouseAssignments?.map(
      ...
      const result = await getAccessibleWarehouses(
...
export const useWarehouseStructure = (warehouseId?: number) => {
  ...
  return useQuery({
    ...
    // ===== FIX START =====
    queryFn: async () => {
      const structureData = await getWarehouseStructure(
        companyId!,
        warehouseId!,
      );
