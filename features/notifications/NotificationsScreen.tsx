...
export default function NotificationsScreen() {
  ...
  const handleNotificationPress = (item: Notification) => {
    ...
    switch (item.referenceType) {
      case 'InboundOrder':
        if (isStaff) {
          router.push(`/(tabs)/tasks/inbound/${id}`);
        } else {
          router.push(`/(tabs)/tasks/inbound/${id}`);
        }
        break;
      case 'OutboundOrder':
        if (isStaff) {
          router.push(`/(tabs)/tasks/outbound/${id}`);
        } else {
          router.push(`/(tabs)/tasks/outbound/${id}`);
        }
        break;
      case 'StockCountTicket':
        ...
        router.push(`/(tabs)/tasks/count/${id}`);
        break;
      case 'InboundRequest':
        // Requests are for managers
        router.push(`/(tabs)/tasks/inbound/${id}`); // Redirect to inbound tasks as placeholder
        break;
      default:
